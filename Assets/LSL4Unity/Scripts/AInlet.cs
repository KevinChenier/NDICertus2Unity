...
namespace Assets.LSL4Unity.Scripts.AbstractInlets
{
	public abstract class AFloatInlet : MonoBehaviour
	{
		...
		IEnumerator ResolveExpectedStream()
		{
            ...
            //Debug.LogWarning(string.Format("Found {0} inlets!", results.Length));
            /*
            for (int i = 0; i < results.Length; i++)
            {
                liblsl.StreamInlet _inlet = new liblsl.StreamInlet(results[i]);
                float[] _sample = new float[_inlet.info().channel_count()];
                _inlet.pull_sample(_sample);
                if (_sample != null)
                {
                    Debug.LogWarning("Found valid inlet at index: " + i);
                    inlet = _inlet;
                }
            ...
            */
            ...
            //liblsl.StreamInlet[] inlets = new liblsl.StreamInlet[results.Length]; 

			inlet = new liblsl.StreamInlet(results[results.Length - 1]);
            //Debug.LogWarning(inlet.samples_available());
            //Debug.Log();
